...
from .lora import MultiModalLoRA_CLIP, MLPFusionLoRA_CLIP, NoisyTopkRouter
...
class CLIPVisionTransformer(nn.Module):
    ...
    def add_init_qkv_lora(self):
        self.lora_layers = list(range(len(self.transformer.resblocks)))
        ...
        for i, block in enumerate(self.transformer.resblocks):
            ...
            self.w_a_v = []
            self.w_b_v = []
            w_q_linear = block.attn.q_proj_weight
            w_v_linear = block.attn.v_proj_weight

            dim_in = w_q_linear.in_features
            ...
            block.attn.qkv = MultiModalLoRA_CLIP( # MultiModalLoRA MultiModalHiRA 
                len(self.modals),
                w_q_linear,
                w_v_linear,
                i_w_a_linear_q,
            ...
            block.mlp = MLPFusionLoRA_CLIP( # MLPFusionHiRA MLPFusionLoRA MLPFusionLoRA_wo_router
                len(self.modals),
                w_mlp,
                i_w_a_linear_fc1,
                i_w_b_linear_fc1,
    ...
    def load_state_dict(self, path=None, strict=True):
        ...
        if isinstance(path, str):
            checkpoint = torch.jit.load(path, map_location='cpu').float().state_dict()
            ...
            if 'positional_embedding' in state_dict.keys():
                if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                    ...
                    spatial_pos = F.interpolate(state_dict["positional_embedding"][1:,].reshape(1, 14, 14, self.embed_dim).permute(0, 3, 1, 2), size=self.spatial_size, mode='bilinear')
            ...
            patch_embed_proj = state_dict['conv1.weight']
            state_dict['conv1.weight'] = torch.nn.functional.interpolate(
                patch_embed_proj.float(), size=self.patch_size, mode='bicubic', align_corners=False)

            msg = super().load_state_dict(state_dict, strict=strict)
            print(msg)
...
class CLIP_B(CLIPVisionTransformer):
    def __init__(self, input_resolution=512, patch_size=16, modals=['rgb', 'depth', 'event', 'lidar'], **kwargs):
        super(CLIP_B, self).__init__(
            input_resolution=input_resolution, patch_size=patch_size, modals=modals, width=768, layers=12, output_dim=512, heads=12, out_indices=[7, 11, 15, 23], \
            drop_path_rate=0.1, get_embeddings=False, **kwargs)    
    
class CLIP_L(CLIPVisionTransformer):
    def __init__(self, input_resolution=512, patch_size=16, modals=['rgb', 'depth', 'event', 'lidar'], **kwargs):
        super(CLIP_L, self).__init__(
            input_resolution=input_resolution, patch_size=patch_size, modals=modals, width=1024, layers=24, output_dim=768, heads=16, out_indices=[7, 11, 15, 23], \
            drop_path_rate=0.1, get_embeddings=False, **kwargs)