...
from semseg.models.backbones.MixT import mit_b0, mit_b1, mit_b2, mit_b4
...
class BaseModel(nn.Module):
    def __init__(self, backbone: str = 'MiT-B0', img_size: int = 512, patch_size: int = 14, num_classes: int = 19, modals: list = ['rgb']) -> None: #, 'depth', 'event', 'lidar'
        super().__init__()
        # backbone, variant = backbone.split('-')
        # self.backbone = eval(backbone)(variant, modals)
        if 'DINOv2' in backbone or 'CLIP' in backbone:
            self.backbone = eval(backbone)(img_size, patch_size, modals)
        elif backbone == 'mit_b0':
            self.backbone = mit_b0()
            state_dict = torch.load('checkpoints/pretrained/segformer/mit_b0.pth')
            ...
            state_dict = torch.load('checkpoints/pretrained/segformer/mit_b1.pth')
            state_dict.pop('head.weight')
            state_dict.pop('head.bias')
            msg = self.backbone.load_state_dict(state_dict,strict=False)            
            print(msg)
            ...
            state_dict = torch.load('checkpoints/pretrained/segformer/mit_b2.pth')
            state_dict.pop('head.weight')
            state_dict.pop('head.bias')
            msg = self.backbone.load_state_dict(state_dict,strict=False)            
            print(msg)
            ...
            state_dict = torch.load('checkpoints/pretrained/segformer/mit_b4.pth')
            state_dict.pop('head.weight')
            state_dict.pop('head.bias')
            msg = self.backbone.load_state_dict(state_dict,strict=False)            
            print(msg)
