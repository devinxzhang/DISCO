...
def cal_flops(model, img_size, modals, logger):
    x = [torch.zeros(1, 3, img_size, img_size) for _ in range(len(modals))]
    # x = [torch.zeros(2, 3, img_size, img_size) for _ in range(len(modals))] #--- PGSNet
    # x = [torch.zeros(1, 3, img_size, img_size) for _ in range(len(modals))] # --- for HRFuser
    if torch.distributed.is_initialized():
        if 'HR' in model.module.__class__.__name__:
            x = [torch.zeros(1, 3, img_size, img_size) for _ in range(len(modals))] # --- for HorNet
    else:
...
class BlockMaskGenerator:
    ...
    def __call__(self, imgs):
        M, B, C, H, W = imgs.shape
        rand_N = np.random.randint(1, M + 1) 
        sel_img = imgs[torch.randperm(M)][:rand_N]
        ...
        if rand_N == 1:
            ...
            input_mask = F.interpolate(input_mask, size=(H, W), mode='nearest').unsqueeze(0)
        else:
            mshape = rand_N, B, round(H / self.mask_block_size), round(W / self.mask_block_size)
            input_mask = torch.rand(mshape, device=imgs.device)
            threshold = torch.linspace(0, 1, steps=rand_N+1, device=imgs.device)
            input_mask = ((input_mask >= threshold[:-1].view(rand_N, 1, 1, 1)) & \
                (input_mask < threshold[1:].view(rand_N, 1, 1, 1))).float()
            input_mask = F.interpolate(input_mask, size=(H, W), mode='nearest').unsqueeze(2)
        
        # sel_img = imgs[torch.randperm(M)]
        # mshape = M*B, 1, round(H / self.mask_block_size), round(W / self.mask_block_size)
        # input_mask = torch.rand(mshape, device=imgs.device)
        # input_mask = (input_mask > self.mask_ratio).float()
        
        # input_mask = F.interpolate(input_mask, size=(H, W), mode='nearest')
        masked_imgs = (sel_img * input_mask).sum(0)
        return masked_imgs