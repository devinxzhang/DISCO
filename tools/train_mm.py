...
def main(cfg, gpu, save_dir):
    ...
    model = eval(model_cfg['NAME'])(model_cfg['BACKBONE'], model_cfg['TEXT_ENCODER'], train_cfg['IMAGE_SIZE'][0], model_cfg['PATCH_SIZE'], trainset.n_classes, dataset_cfg['MODALS'])
    # model = eval(model_cfg['NAME'])(model_cfg['BACKBONE'], trainset.n_classes, dataset_cfg['MODALS'], model_cfg['MASK_RATIO'], model_cfg['MASK_BLOCK_SIZE'])
    resume_checkpoint = None
    if os.path.isfile(resume_path):
        model.backbone.add_init_qkv_lora()
        resume_checkpoint = torch.load(resume_path, map_location=torch.device('cpu'))
        msg = model.load_state_dict(resume_checkpoint['model_state_dict'], strict=False)
        logger.info(msg)
    else:
        # model.init_pretrained(model_cfg['PRETRAINED'])
        # model.backbone.load_state_dict(torch.load(model_cfg['PRETRAINED'], map_location='cpu'), strict=False)
        msg = model.backbone.load_state_dict(model_cfg['PRETRAINED'], strict=False)
        print(msg)
        model.backbone.add_init_qkv_lora()        
        # model.backbone.add_init_proj_lora()
        # msg = model.load_state_dict(torch.load(model_cfg['PRETRAINED'], map_location='cpu'), strict=False)
    ...
    if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
        ...
        # cal_flops(model, train_cfg['IMAGE_SIZE'][0], dataset_cfg['MODALS'], logger)
    ...
    # total = sum(p.numel() for p in model.backbone.parameters() if p.requires_grad)            
    # print(f"Total parameters:     {total / 1e6:.2f}M")
    # for name, params in model.named_parameters():
    #     print(name, '--', params.numel(), 'requires_grad:', params.requires_grad) 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # pbar = tqdm(enumerate(trainloader), total=iters_per_epoch*accum_steps, desc=f"Epoch: [{epoch+1}/{epochs}] Iter: [{0}/{iters_per_epoch}] LR: {lr:.8f} Loss: {train_loss:.8f}")

        for iter, (sample, lbl) in pbar:
            optimizer.zero_grad(set_to_none=True)
            # do_sync = ((iter + 1) % accum_steps == 0) or ((iter + 1) == len(trainloader))
            ...
            # with model.no_sync() if not do_sync else nullcontext():
            #     with autocast(enabled=train_cfg['AMP']):
            #         logits = model(sample)
            ...
            # pbar.set_description(f"Epoch: [{epoch+1}/{epochs}] Iter: [{math.ceil((iter+1)/accum_steps)}/{iters_per_epoch}] LR: {lr:.8f} Loss: {train_loss / ((iter+1)/accum_steps):.8f}")
...
if __name__ == '__main__':
    ...
    exp_name = '_'.join([cfg['DATASET']['NAME'], model, modals, 'debug'])
