...
class SemSeg:
    def __init__(self, cfg) -> None:
        ...
        self.model = eval(cfg['MODEL']['NAME'])(cfg['MODEL']['BACKBONE'], cfg['MODEL']['TEXT_ENCODER'], cfg['EVAL']['IMAGE_SIZE'][0], cfg['MODEL']['PATCH_SIZE'], len(self.labels), cfg['DATASET']['MODALS'])
        self.model.backbone.add_init_qkv_lora()
...
if __name__ == '__main__':
    ...
    save_dir = Path(cfg['SAVE_DIR']) / 'test_results' / (cfg['DATASET']['NAME']+'_'+cfg['MODEL']['BACKBONE']+'_'+modals_name+'_'+'d')
    ...
    if test_file.is_file():
        ...
        for file in files:
            ...
            from torchvision import io
            gt_file = file.replace('/img', '/semantic').replace('_rgb', '_semantic')
            gt = io.read_image(gt_file)[0,...].long()
            gt[gt==255] = 0
            gt -= 1
            gt_palette = eval(cfg['DATASET']['NAME']).PALETTE
            gt_palette = torch.concat([gt_palette, torch.zeros(1, 3)], dim=0)
            gt_rgb = gt_palette[gt]
            gt_rgb = gt_rgb.to(torch.uint8)
            pil_image = Image.fromarray(gt_rgb.numpy()).convert("RGB")
            # save_path = os.path.join(str(save_dir),file.replace('/img', '/semantic').replace('_rgb', '_semantic'))
            save_path = 'output/test_results/073950.png'
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
