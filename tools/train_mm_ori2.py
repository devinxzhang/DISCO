...
from semseg.models.utils import gradient_conflict_minimization
...
def main(cfg, gpu, save_dir):
    ...
    trainloader = DataLoader(trainset, batch_size=train_cfg['BATCH_SIZE'], num_workers=8, drop_last=True, pin_memory=False, sampler=sampler)
    ...
    #     # if 'linear_a_q' not in name and 'linear_b_q' not in name and 'linear_a_v' not in name and 'linear_b_v' not in name and \
    #     #     'linear_a_fc1' not in name and 'linear_b_fc1' not in name and 'linear_a_fc2' not in name and 'linear_b_fc2' not in name and \
    #     #     'gate' not in name:
    #     #     params.requires_grad = False
    #     if 'fpn' in name or 'decode_head' in name:
    #         params.requires_grad = False
    for name, params in model.named_parameters():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        #######################################
        # model.use_masking = False
        if epoch < 50:
            model.use_masking = False
        else:
            if (epoch % 10) < 5:
                model.use_masking = True
                for name, params in model.named_parameters():
                    if 'linear_a_q' not in name and 'linear_b_q' not in name and 'linear_a_v' not in name and 'linear_b_v' not in name and \
                        'linear_a_fc1' not in name and 'linear_b_fc1' not in name and 'linear_a_fc2' not in name and 'linear_b_fc2' not in name and \
                        'gate' not in name:
        ...
        for iter, (sample, lbl, sample_id) in pbar:
...
if __name__ == '__main__':
    ...
    exp_name = '_'.join([cfg['DATASET']['NAME'], model, modals, 'ddp8'])
