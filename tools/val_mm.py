...
def evaluate(model, dataloader, device):
    ...
    sliding = False
    for images, labels, images_id in tqdm(dataloader):
...
def main(cfg):
    ...
    for case in cases:
        ...
        model = eval(cfg['MODEL']['NAME'])(cfg['MODEL']['BACKBONE'], cfg['MODEL']['TEXT_ENCODER'], cfg['EVAL']['IMAGE_SIZE'][0], cfg['MODEL']['PATCH_SIZE'], dataset.n_classes, cfg['DATASET']['MODALS'])
        model.backbone.add_init_qkv_lora()
        # msg = model.load_state_dict(torch.load(str(model_path), map_location='cpu'))
        msg = model.init_pretrained(str(model_path))
        print(msg)
        model = model.to(device)
        sampler_val = None
        dataloader = DataLoader(dataset, batch_size=eval_cfg['BATCH_SIZE'], num_workers=8, pin_memory=False, sampler=sampler_val)
        if True:
